...
filename_contains="Locs.csv"

# Folders to analyse:
root_path=r"/Users/Mathew/Documents/Edinburgh Code/Lengths_new_ONI.py/"
pathlist=[]
...
pathlist.append(r"/Users/Mathew/Documents/Edinburgh Code/Lengths_new_ONI.py/Test_data/")
...
def generate_SR_cluster(coords,clusters):
    SR_plot_cluster=np.zeros((image_width*scale,image_height*scale),dtype=float)
    SR_plot_def=np.zeros((image_width*scale,image_height*scale),dtype=float)
    j=0
    for i in coords:
        if clusters[j]>-1:
            ...
            SR_plot_cluster[scale_ycoord,scale_xcoord]+=clusters[j]+1
            
        j+=1
        
    SR_plot_def_one=np.copy(SR_plot_def)
    # print('done copy')
    SR_plot_def_one[SR_plot_def_one==0]=1
    # print('replace nums')
    SR_plot_cluster=SR_plot_cluster/SR_plot_def_one
    # print('done divide')
    return SR_plot_def,SR_plot_cluster
...
for path in pathlist:
    ...
    index_names = loc_data[loc_data['X precision (nm)']>prec_thresh].index
    loc_data.drop(index_names, inplace = True)
    
    index_names = loc_data[loc_data['Y precision (nm)']>prec_thresh].index
    loc_data.drop(index_names, inplace = True)
   


    # Extract useful data:
    coords = np.array(list(zip(loc_data['X (nm)']/Pixel_size,loc_data['Y (nm)']/Pixel_size)))
    precsx= np.array(loc_data['X precision (nm)'])
    precsy= np.array(loc_data['Y precision (nm)'])
    xcoords=np.array(loc_data['X (nm)']/Pixel_size)
    ycoords=np.array(loc_data['Y (nm)']/Pixel_size)
    ...
    if to_cluster==1:
        ...
        if len(cluster_contents)>0:
            ...
            SR_Clu,labelled=generate_SR_cluster(coords,clusters)
            
            imsr = Image.fromarray(SR_Clu)
            imsr.save(path+'SR_points_python_clustered.tif')
            
            SR_clu_prec,labelled_w,SR_prec_plot=generate_SR_prec_cluster(coords,precsx,precsy,clusters)
            ...
            imsr = Image.fromarray(labelled_w)
            imsr.save(path+'SR_fwhm_python_clustered.tif')
            
            imsr = Image.fromarray(labelled)
            imsr.save(path+'SR_points_python_cluster_number.tif')
        ...
        if to_cluster==1:
            Output_all_cases.to_csv(root_path + 'GDSC_all_metrics.csv', sep = '\t')
